...
public class S69 { // 뮤직비디오 (결정알고리즘) -> dvd 최소용량 구하기
	public int count(int[] arr, int capacity) { //몇 장의 dvd를 사용하는지 구하기
		int cnt = 1, sum = 0;
	...
	public int solution(int n, int m, int[] arr) {
		...
		int lt = Arrays.stream(arr).max().getAsInt(); // arr[]의 최댓값을 lt로 설정
		int rt = Arrays.stream(arr).sum(); // 한 dvd에 배열이 전부 들어갈 수 있는 용량(총합)을 rt로 설정
