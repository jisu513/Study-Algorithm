...
public class S222 {// 멘토링
	...
	public int solution(int n, int m, int[][] arr) {// n은 학생 수, m은 test수
		...
		for (int i = 1; i <= n; i++) {// n:학생 수 i=멘토
			for (int j = 1; j <= n; j++) {// n:학생 수 j=멘티 --> (i, j) = (멘토, 멘티) 경우의 수
				int cnt = 0;
				for (int k = 0; k < m; k++) {// m:test 수
					int pi = 0, pj = 0;
					for (int s = 0; s < n; s++) {// n:등 수 --> 학생 수 = 등수(당연한 소리지만.. 헷갈림)
					...
					if (pi < pj) {// 등수는 값이 높은 등수!(헷갈림 주의)
						cnt++;
					}
				}
				if (cnt == m) {// cnt의 값과 test수가 같아야 모든 test에서 i가 j보다 높은 등수 --> 멘토/멘티 가능!
					answer++;
				}
